...
async function fetchBlogPosts(authors?: string, locale: string = 'pt') {
  ...
  const response = await fetch(`${API_BASE_URL}/spaces/${env.CONTENTFUL_SPACE_ID}/environments/${env.CONTENTFUL_ENVIRONMENT}/entries?content_type=blogPost&${authorFilter}&locale=${encodeURIComponent(locale)}`, {
    next: {
...
export default async function BlogPage(
  { searchParams, params }: { searchParams: Promise<{ authors?: string, categories?: string }>, params: Promise<{lang: string}> }
) {
  const [{
    lang,
  }, {
  ...
  const { items, includes } = await fetchBlogPosts(authors, lang)
